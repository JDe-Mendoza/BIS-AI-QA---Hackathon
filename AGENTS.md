...
3. **Never create Todo work.** `swb new` lands in Backlog. Only a human promotes
   Backlog -> Ready.
...
`recordCompletion` is idempotent per `(learner, activityId)`; `getMyTopics`
returns granted-only. SCORM completes via runtime callback, not the open-endpoint.
...
```json
{ "teamKey": "HAC", "testCommand": "npx vitest run", "defaultBranch": "<team branch>" }
```
...
- `testCommand` - run by `swb done`; non-zero blocks the transition. QA owns
  this. This QA workspace uses Vitest (`npx vitest run`) for the agent's own
  acceptance/contract suite (`tests/`, scoped to the Support MDs + CONTRACTS.md).
  Note the layer split: Vitest = JS/React unit + contract; Playwright
  (`../hackathon-automation/`) = web E2E on staging; dev repo `team2-hackathon`
  runs Jest (`npx jest`, NOT `node --test` - see `tasks/lessons.md`).
...
- **Team branch / `defaultBranch`:** TBD - confirm with captain.
- **Reading dev code:** the QA agent needs a view of the `team2-hackathon` code
  to repoint specs from the contract oracle to real modules (mechanism TBD with
  the human - clone / shared folder / pasted diffs).
