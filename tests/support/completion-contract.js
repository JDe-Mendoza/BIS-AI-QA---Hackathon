...
// before the real learner module is readable. When the team2-hackathon clone is
...
export function createCompletionStore(now = () => new Date().toISOString()) {
  ...
  // Topic membership is normally resolved server-side; for the oracle we key
  // activityIds to a topic via a simple registry so per-topic counts are honest.
