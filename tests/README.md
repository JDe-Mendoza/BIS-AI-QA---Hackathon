...
Once that repo is cloned and mounted (see the `code` alias in `vitest.config.js`):
...
2. The assertions **do not change** — if the real implementation is correct, the
   same guard tests stay green. That repoint is the definition-of-done gate.

The agent reads what the devs changed via `git -C <clone> pull` + `git log`/`git diff`
