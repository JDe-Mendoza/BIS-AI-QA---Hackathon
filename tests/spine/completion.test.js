// Suite I — Topic page & completion logic (T0, the core). ⭐ = stop-the-line guard.
...
// team2-hackathon learner module once the clone is wired (see tests/README.md);
...
describe('Suite I — completion (T0)', () => {
  ...
  it('I7 ⭐ — re-opening a completed item does NOT double-count (no 4/3, no regression)', () => {
    open(store, LEARNER_X, A1);
  ...
  it('I8 ⭐ — completion is per-learner; X progress never leaks to Y', () => {
    open(store, LEARNER_X, A1);
    open(store, LEARNER_X, A2);
  ...
  it('I9 ⭐ — completion survives reload / re-login (persisted, not session-only)', () => {
    open(store, LEARNER_X, A1);
  ...
  // Behaviours that need the real FE/endpoint or the dev clone — write once code is readable.
  ...
  it.todo('I11 ⭐ [T2] admin adds new content → topic moves Completed → In Progress, "● Updated" pill');
