...
// separately in tests/spine/completion.test.js.
//
// Field constraints (load-bearing, from the V2 spec):
//   Topic: title ≤150 required · description ≤500 required · image JPG/PNG <4MB · status Active(default)/Draft/Deactivated
//   Content: title ≤50 required · type Link/PDF/Video(URL+MP4 <80MB)/SCORM · sorted alphabetically · completeAgain edit-only
...
describe('Suite D — Content lifecycle: deactivate/reactivate/purge/bulk [T1]', () => {
  ...
  it.todo('D4 ⭐ purge (purge code) → permanent delete AND all completion records removed');
  it.todo('D5–D7 bulk-select action bars + select-all');
});

describe('Suite E — Edit extras: drag-reorder & Complete Again [T1/T2]', () => {
  it.todo('E4 [T1] ⭐ Preview (eye) must NOT record completion for the admin');
...
describe('Suite L — Unavailable topic/content [T1]', () => {
  it.todo('L1 web deactivated/purged link → "⚠️ Cannot be found" modal after login');
...
describe('Suite N — Mobile (SafeTapp) [T1] — executed manually per TEST-STRATEGY (Playwright is web-only)', () => {
  ...
  it.todo('N6 ⭐ completion syncs both ways mobile↔web (shared completion API)');
});
